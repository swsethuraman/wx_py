from wx.providers.market_data import Bloomberg_Provider as BBG
from flask_script import Command, Option
import datetime
from datetime import datetime
import dateutil.relativedelta

class BBG_SECMASTER_Command(Command):
    """
        This is the command line interface for saving DA LBMP data from NYISO at the end of every day.
    """
    ...
    def run(self, as_of_date='today', sys='unix'):
        ...
        start = start - dateutil.relativedelta.relativedelta(days=365)
        end = end + dateutil.relativedelta.relativedelta(days=1)
        start = datetime.strftime(start, '%Y%m%d')
        end = datetime.strftime(end, '%Y%m%d')
        BBG.bloomberg_secmaster(db, start_date=start, end_date=end)
        pass
